...
class WalletsPlugin(Plugin):
    ...
    def _kick_import_ens_lookups(self, addresses: list[str]) -> None:
        """One ENS reverse-lookup worker per just-imported address.
        ...
        for addr in addresses:
            ...
            w.resolved.connect(self._on_import_ens_resolved)
    ...
    def _on_import_ens_resolved(self, address: str, name: str) -> None:
        """ENS worker came back. Overwrite the imported account's
        label IFF a real (forward-verified) name resolved AND the
        user hasn't manually edited the label since the import
        (we check by reading the account's current label and
        only replacing the auto-generated short-id form, not
        anything that doesn't look like Frame's
        ``deadbeef…`` / ``deadbeef… (key 1/2)`` pattern)."""
        if not name:
            return
