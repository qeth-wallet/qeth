...
# The source key ("ledger" / "trezor" / "hot" / "watch_only" / "qr") on a top-level branch
...
_SECTIONS: list[tuple[str, str, str, bool]] = [
    ...
    ("trezor", "Trezor", "act_add_trezor", True),
...
class WalletsPlugin(Plugin):
    ...
    def _build_account_actions(self) -> None:
        ...
        self.act_add_trezor = QAction("&Trezor Account…", self)
        self.act_add_trezor.setIcon(_icon(
            "drive-removable-media", "media-removable", "drive-removable-media-usb",
            QStyle.StandardPixmap.SP_DriveFDIcon,
        ))
        self.act_add_trezor.triggered.connect(self._add_trezor)
        ...
        self._add_menu.addAction(self.act_add_trezor)
    ...
    def _remove_selected_account(self) -> None:
        ...
        if sources <= {"ledger", "trezor", "qr"}:
            consequence = (
                "Keys on your hardware wallet are untouched; this only "
                "forgets the addresses locally. You can re-add them via "
                "Scan at any time."
            )
        elif sources == {"hot"}:
            consequence = (
            ...
            consequence = (
                "Hardware-wallet and watch-only entries are reversible (re-scan "
    ...
    def _add_trezor(self) -> None:
        """Scan a connected Trezor for accounts on a derivation scheme and add
        the ones the user picks. The wallet's root fingerprint (distinct per
        seed and passphrase) keys its device tree, like a QR wallet's."""
        if self.host is None:
            return
        dlg = AddTrezorDialog(
            self.host.current_chain(), self._container,
            existing_addresses=self._addresses_for_source("trezor"))
        if dlg.exec() != QDialog.DialogCode.Accepted:
            return
        scheme = dlg.scheme_combo.currentData()   # the key, not the full-path text
        xfp = dlg.fingerprint
...
class AddTrezorDialog(AddLedgerDialog):
    """The Ledger scan-list / select UI, discovering through a connected Trezor
    (``TrezorWorker``). Keeps the wallet's root ``fingerprint`` from the scan so
    the add flow can key the device tree on it."""

    def __init__(self, chain, parent=None, existing_addresses=None):
        super().__init__(chain, parent, existing_addresses=existing_addresses)
        from ...trezor import PATH_SCHEMES as TREZOR_SCHEMES
    ...
    def _scan(self) -> None:
        ...
        from ...trezor import TrezorWorker
        self.results.clear()
        self.add_btn.setEnabled(False)
        n = self.count_spin.value()
        if n == 0:
        ...
        self.progress.setVisible(True)
        self.scan_btn.setEnabled(False)
        # Hosts the device's prompts during the scan: "enter your PIN on the
        # Trezor" if it's locked, or a PIN / passphrase dialog on a Trezor One.
        self._interaction = DialogInteraction(self, title="Trezor")
        worker = TrezorWorker(
            self.scheme_combo.currentData(), n, chain=self._chain,
            ui=self._interaction)
        worker.fingerprint.connect(self._on_fingerprint)
        worker.discovered.connect(self._on_found)
        worker.finished_ok.connect(self._on_done)
        worker.failed.connect(self._on_failed)
        self._workers.add(worker)
    ...
    def _on_fingerprint(self, xfp: str) -> None:
