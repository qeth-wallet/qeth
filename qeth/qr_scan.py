...
class CameraScanner(QObject):
    ...
    def __init__(self, parent: QObject | None = None) -> None:
        ...
        # Set before any start(): the permission callback below fires
        # asynchronously and must not start a camera the dialog already closed.
    ...
    def start(self) -> None:
        ...
        calling ``QCamera.start()`` there only logs "Access to camera not
        granted" and never reports an error, which is why the dialog sat at
        "Starting camera…" forever (PR #25). Every other desktop keeps the
        direct start — including Linux distributions whose system Qt predates
        ``QCameraPermission``, which is why the import is inside the branch."""
    ...
    def _permission_decided(self, permission: Any) -> None:
        """Continue an asynchronous permission request — but only while the
        dialog still wants the camera (``stop()`` clears the flag, so a grant
        that lands after the user closed the dialog doesn't open the camera)."""
    ...
    def _emit_permission_denied(self) -> None:
        self.failed.emit(
            "Camera access denied. Allow qeth to use the camera in "
            "System Settings."
        )
