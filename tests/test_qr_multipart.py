...
def test_fragmentation_sizing_matches_the_spec():
    ...
    parts = encode_parts("t", bytes(256), max_fragment_len=30)
    assert len(parts) == 9


def test_small_payload_is_a_single_plain_part():
    parts = encode_parts("eth-sign-request", bytes(range(120)), max_fragment_len=150)
    assert len(parts) == 1
...
def test_large_payload_animates_and_roundtrips():
    ...
    parts = encode_parts("eth-sign-request", msg, max_fragment_len=150)
...
def test_roundtrip_across_sizes_including_padding():
    for size in (1, 149, 150, 151, 299, 1000):
        ...
        assert decode_parts(encode_parts("x", msg, max_fragment_len=150)) == ("x", msg)


def test_encoded_part_carries_the_right_fields():
    msg = bytes(range(200))
    parts = encode_parts("x", msg, max_fragment_len=150)
...
def test_decode_rejects_incomplete():
    parts = encode_parts("x", bytes(range(200)), max_fragment_len=150)
    import pytest
